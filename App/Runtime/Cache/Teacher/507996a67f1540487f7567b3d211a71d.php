                    ...
                    <?php if(is_array($allhomework)): foreach($allhomework as $key=>$v): ?><div class="row">
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <?php if(($v["htype"] == 2) or ($v["htype"] == 3) ): ?><a href="<?php echo U('Teacher/Homework/detailnew',array('cno' => $_REQUEST['cno'],'cname' => $_REQUEST['cname'],'hno' => $v['hno']));?>" class="title" target="_blank"><?php echo ($v["htitle"]); ?>
                                        </a>
                                    <?php else: ?>                                    
                                        <a href="<?php echo U('Teacher/Homework/detail',array('cno' => $_REQUEST['cno'],'cname' => $_REQUEST['cname'],'hno' => $v['hno']));?>" class="title" target="_blank"><?php echo ($v["htitle"]); ?>
                                        </a><?php endif; ?>
                                    </br>
                                    ...
                                    <form class="form-delete inline">
                                        ...
                                        <a class="pull-right btn btn-xs btn-edit" href="<?php echo U('Teacher/Homework/edit',array('cno' => $_REQUEST['cno'],'cname' => $_REQUEST['cname'],'hno'=>$v['hno']));?>">编辑</a>
                                        <?php if($v['hfreeze'] == 0): ?><a class="pull-right btn btn-xs btn-freeze">冻结</a>  
                                        <?php else: ?> 
                                            <a class="pull-right btn btn-xs btn-unfreeze">解冻</a><?php endif; ?>        
                                    </form>
                                </div>
                            </div>
           ...
           $('#datetimepicker1').datetimepicker({
            ...
            $("#btn-homework").click(function(event) {
                ...
                $("#form-homework").ajaxSubmit({
                    url:"http://202.112.113.119/OBELearn/teacher/upload/addHomeworkHandle.html",
                    type:"POST",
                    data: {
            ...
            $(".btn-delete").click(function(event) {
                ...
                $.post("<?php echo U('Teacher/Homework/deleteHomework');?>", $s.closest('form').serialize(), function(data, textStatus, xhr) {
            ...
            $(".btn-freeze").click(function(){
                ...
                var hno = $(this).prev().prev().prev().val();
                $.post("<?php echo U('Teacher/Homework/freeze');?>",{cno:cno,hno:hno},function(data){
                    if(data.status == 1){
                        alert("冻结成功，该作业将不能再被提交，删除与重交");
                        CAR(1);
                    }
                    else{
                        alert("冻结失败，请联系管理员！");
            ...
            $(".btn-unfreeze").click(function(){
                var cno = $(this).prev().prev().prev().prev().val();
                var hno = $(this).prev().prev().prev().val();
                $.post("<?php echo U('Teacher/Homework/unfreeze');?>",{cno:cno,hno:hno},function(data){
                    if(data.status == 1){
                        alert("解冻成功，该作业将可以继续被提交，删除与重交");
                        CAR(1);
                    }
                    else{
                        alert("解冻失败，请联系管理员！");
