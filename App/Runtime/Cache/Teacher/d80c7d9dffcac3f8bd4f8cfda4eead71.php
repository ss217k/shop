                ...
                <div id="main-right">
                    <div class="panel panel-primary" style="min-width:600px">
                        ...
                        <div class="panel-body">
                            ...
                            <div>    
                                <div class="panel-body">
                                    成绩分布图：<input id = "display-graph" type="button" class="btn-mark" fno="<?php echo ($v["fno"]); ?>" sno="<?php echo ($v["sno"]); ?>" hno="<?php echo I('hno');?>" value="展示统计图">
                                    <div style="width:45%;margin-bottom:-5%;position:relative;left:30%">
                                        <canvas id="canvas1"></canvas>
                                    </div>
                                    <div style="width:55%;margin-bottom:-5%;position:relative;left:30%;bottom: 0px">
  ...
  s.parentNode.insertBefore(hm, s);
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $.post("<?php echo U('Teacher/Homework/lookup');?>",{hno:$("#hnohelp").val(),cno:$("#cnohelp").val()},function(res){
                    if(res.status == 0)
                    {
                        $("#display-graph").val("展示统计图");
                    }
                    else{
                        $("#display-graph").val("隐藏统计图");
                    }
                    // var dat = res.data;
...
/*
            ...
            $("#display-graph").click(function(){
                $.post("<?php echo U('Teacher/Homework/getLabel');?>",{hno:$("#hnohelp").val(),cno:$("#cnohelp").val()},function(res){
                    if(res.status == 0)
                    {
                        $("#display-graph").val("展示统计图");
                    }
            ...
            function init_chart(){
                ...
                $.post("<?php echo U('Teacher/Homework/getGradeProportion');?>",{labels:config2.data.labelsbound,hno:$("#hnohelp").val(),cno:$("#cnohelp").val()},function(res){
                    console.log(res);
                    if(res.status != 0)
                    {
