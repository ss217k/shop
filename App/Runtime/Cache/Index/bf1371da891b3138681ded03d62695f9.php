 ...
 foreach (session('selectedCourses') as $key => $value) { ?>
	...
	<div class="panel-body">
		...
		<div>
			...
			<span class="title">				
				<a href="<?php echo U('Index/Question/questionDetail',array('qno' =>$v['qno'],'cno'=>I('cno')));?>" target="_blank"><?php echo ($v["qtitle"]); ?></a>
			</span>
		</div>
		...
		<div class="question-bar">
			<form class="questionnumber">
				...
				<?php if($v['isFocus']==0): ?><a class="focus btn btn-success btn-xs">关注</a>
					...
					<span class="attnum"><?php echo ($v["attnum"]); ?></span>
					人关注)<?php endif; ?>
				<a  href="<?php echo U('Index/Question/questionDetail',array('qno' =>$v['qno'],'cno'=>I('cno')));?>" target="_blank"><?php echo ($v["rplynum"]); ?>个回答</a>
...
(function() {
  ...
  <script>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            url:"<?php echo U('Index/Question/createQuestion');?>",
            type:'POST',
            dataType:'json',
  ...
  <script type="text/javascript">
    function focus($s){
      $.post("<?php echo U('Index/Question/focus');?>",
            $s.parent().serialize(),
            function(data) {
           if (data.status == 1) {
    ...
    function unfocus($s){
      $.post("<?php echo U('Index/Question/unFocus');?>",
            $s.parent().serialize(),
            function(data) {
            if (data.status == 1) {
    ...
    $(document).ready(function(){
      $(".cancelfocus").click(function() {
        ...
        $(".btn-delete").click(function() {
          $s=$(this);
          $.post("<?php echo U('Index/Question/deleteQuestion');?>",
            $s.parent().serialize(),
            function(data) {
            if (data.status == 1) {
